...
4. Check for established workflows:
   - Read CLAUDE.md (if exists) for project-specific workflows
   - Look for `.claude/skills/` directory
   - Match file paths to domain patterns:
     - `app/models/`, `app/controllers/` → Rails workflow
     - `src/components/`, `.tsx` files → React workflow

5. Present action options to user:
   - **If matching skill/workflow found**: "This looks like [Rails/React] work. Would you like to:\n\n1. Invoke [skill-name] skill and start\n2. Work on it directly\n3. Brainstorm approach first\n4. Put it back and browse other todos"
...
6. Handle user choice:
   - **Start working**: Remove todo from TO-DOS.md, then begin work
   - **Brainstorm**: Keep todo in file, discuss approach
   - **Put it back**: Return to step 2 to display full list again
...
3. Fix user auth flow (2025-11-14 09:15)
